/**********************************************************************************************
 *
 *   rlgl v6.0 - A multi-OpenGL abstraction layer with an immediate-mode style
 * API
 *
 *   DESCRIPTION:
 *       An abstraction layer for multiple OpenGL versions (1.1, 2.1, 3.3
 * Core, 4.3 Core, ES 2.0, ES 3.0) that provides a pseudo-OpenGL 1.1
 * immediate-mode style API (rlVertex, rlTranslate, rlRotate...)
 *
 *   ADDITIONAL NOTES:
 *       When choosing an OpenGL backend different than OpenGL 1.1, some
 * internal buffers are initialized on rlglInit() to accumulate vertex data
 *
 *       When an internal state change is required all the stored vertex data is
 * rendered in a batch, additionally, rlDrawRenderBatchActive() could be called
 * to force flushing of the batch
 *
 ...
 * platform Those preprocessor defines are only used on the rlgl module, if
 * OpenGL version is required by any other module, use rlGetVersion() to check
 * it
 *
 ...
 * RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS    4    // Maximum number of texture units
 ...
 *           0.05    // Default projection matrix near cull distance #define
 * RL_CULL_DISTANCE_FAR             4000.0    // Default projection matrix far
 * cull distance
 *
 ...
 * Bound by default to shader location:
 * RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION #define
 * RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD     "vertexTexCoord"    // Bound by
 * default to shader location: RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD
 ...
 * Bound by default to shader location:
 * RL_DEFAULT_SHADER_ATTRIB_LOCATION_TANGENT #define
 * RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD2    "vertexTexCoord2"   // Bound by
 * default to shader location: RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2
 *       #define RL_DEFAULT_SHADER_ATTRIB_NAME_BONEINDICES  "vertexBoneIndices"
 * // Bound by default to shader location:
 * RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEINDICES #define
 * RL_DEFAULT_SHADER_ATTRIB_NAME_BONEWEIGHTS  "vertexBoneWeights" // Bound by
 * default to shader location: RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS
 ...
 * matrix #define RL_DEFAULT_SHADER_UNIFORM_NAME_MODEL       "matModel"  //
 * model matrix #define RL_DEFAULT_SHADER_UNIFORM_NAME_NORMAL      "matNormal"
 ...
 * RL_DEFAULT_SHADER_UNIFORM_NAME_BONEMATRICES "boneMatrices"     // bone
 * matrices #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0  "texture0"    //
 * texture0 (texture slot active 0) #define
 * RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1  "texture1"          // texture1
 * (texture slot active 1) #define RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE2
...
#define RLAPI                                                                  \
    __attribute__((visibility("default"))) // Building the library as a Unix
...
// Security check in case no GRAPHICS_API_OPENGL_* defined
    ...
    !defined(GRAPHICS_API_OPENGL_11) && !defined(GRAPHICS_API_OPENGL_21) &&    \
...
typedef struct rlVertexBuffer {
    ...
    float *normals;        // Vertex normal (XYZ - 3 components per vertex)
                           // (shader-location = 2)
    ...
    unsigned int
        vboId[5]; // OpenGL Vertex Buffer Objects id (5 types of vertex data)
} rlVertexBuffer;

// Draw call type
// NOTE: Only texture changes register a new draw, other state-change-related
// elements are not used at this moment (vaoId, shaderId, matrices), raylib
...
RLAPI void rlSetClipPlanes(double nearPlane,
                           double farPlane); // Set clip planes distances
...
RLAPI void *
rlGetProcAddress(const char *procName);        // Get OpenGL procedure address
...
RLAPI void
rlSetVertexAttribute(unsigned int index, int compSize, int type,
                     bool normalized, int stride,
                     int offset); // Set vertex attribute data configuration
RLAPI void
...
rlLoadTextureCubemap(const void *data, int size, int format,
                     int mipmapCount); // Load texture cubemap data
RLAPI void
...
RLAPI void rlFramebufferAttach(
    unsigned int id, unsigned int texId, int attachType, int texType,
...
// WARNING: Copy and resize framebuffer functionality only defined for software
// backend
RLAPI void rlCopyFramebuffer(
    int x, int y, int width, int height, int format,
    void *pixels); // Copy framebuffer pixel data to internal buffer
RLAPI void rlResizeFramebuffer(int width,
                               int height); // Resize internal framebuffer

// Shaders management
RLAPI unsigned int rlLoadShader(
    const char *code,
    int type); // Load (compile) shader and return shader id (type:
               // RL_VERTEX_SHADER, RL_FRAGMENT_SHADER, RL_COMPUTE_SHADER)
RLAPI unsigned int
rlLoadShaderProgram(const char *vsCode,
                    const char *fsCode); // Load shader from code strings
RLAPI unsigned int rlLoadShaderProgramEx(
    unsigned int vsId,
    unsigned int fsId); // Load shader program, using already loaded shader ids
RLAPI unsigned int
rlLoadShaderProgramCompute(unsigned int csId); // Load compute shader program
RLAPI void
rlUnloadShader(unsigned int id); // Unload shader, loaded with rlLoadShader()
RLAPI void rlUnloadShaderProgram(unsigned int id); // Unload shader program
RLAPI int
rlGetLocationUniform(unsigned int id,
                     const char *uniformName); // Get shader location uniform,
                                               // requires shader program id
RLAPI int
rlGetLocationAttrib(unsigned int id,
...
RLAPI void rlSetUniformMatrices(int locIndex, const Matrix *mat,
                                int count); // Set shader value matrices
RLAPI void
...
// TODO: OpenGL ES 2.0 support shouldn't be platform-dependent, neither require
// GLAD
...
// TODO: Avoid raylib platform-dependent code on rlgl, it should be a completely
// portable library
...
#define RL_DEFAULT_SHADER_ATTRIB_NAME_BONEINDICES                              \
    "vertexBoneIndices" // Bound by default to shader location:
                        // RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEINDICES
#endif
#ifndef RL_DEFAULT_SHADER_ATTRIB_NAME_BONEWEIGHTS
#define RL_DEFAULT_SHADER_ATTRIB_NAME_BONEWEIGHTS                              \
    "vertexBoneWeights" // Bound by default to shader location:
                        // RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS
#endif

#ifndef RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCETRANSFORM
#define RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCETRANSFORM                        \
    "instanceTransform" // Bound by default to shader location:
...
#define RL_DEFAULT_SHADER_UNIFORM_NAME_BONEMATRICES                            \
    "boneMatrices" // bone matrices (required for GPU skinning)
#endif
...
typedef struct rlglData {
    ...
    struct {
        ...
        unsigned int
            currentTextureId; // Current texture id to be used on glBegin
...
static rl_float16
rlMatrixToFloatV(Matrix mat); // Get float array of matrix data
#define rlMatrixToFloat(mat)                                                   \
    (rlMatrixToFloatV(mat).v) // Get float vector for Matrix
...
void rlMultMatrixf(const float *matf) {
    ...
    *RLGL.State.currentMatrix =
        rlMatrixMultiply(mat, *RLGL.State.currentMatrix);
}
...
void rlOrtho(double left, double right, double bottom, double top, double znear,
    ...
    // division by zero, response to it is platform/compiler dependent
    Matrix matOrtho = {0};
...
void rlVertex3f(float x, float y, float z) {
    ...
    // RL_QUADS come in groups of 4 vertices Checking current draw.mode when a
    // new vertex is required and finish the batch only if the draw.mode
    ...
    if (RLGL.State.vertexCounter >
        ...
        if ((RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
            ...
            // comming NOTE: Adding +1 vertex to the check for some safety
    ...
    // Add current normal
    RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
        .normals[3 * RLGL.State.vertexCounter] = RLGL.State.normalx;
    RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
        .normals[3 * RLGL.State.vertexCounter + 1] = RLGL.State.normaly;
    RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
        .normals[3 * RLGL.State.vertexCounter + 2] = RLGL.State.normalz;
...
void rlNormal3f(float x, float y, float z) {
    ...
    float normalz = z;
    if (RLGL.State.transformRequired) {
        normalx = RLGL.State.transform.m0 * x + RLGL.State.transform.m4 * y +
                  RLGL.State.transform.m8 * z;
        normaly = RLGL.State.transform.m1 * x + RLGL.State.transform.m5 * y +
                  RLGL.State.transform.m9 * z;
        normalz = RLGL.State.transform.m2 * x + RLGL.State.transform.m6 * y +
                  RLGL.State.transform.m10 * z;
    }

    // NOTE: Default behavior assumes the normal vector is in the correct space
    // for what the shader expects, it could be not normalized to 0.0f..1.0f,
    // magnitud can be useed for some effects
    /*
...
void rlSetTexture(unsigned int id) {
    if (id == 0) {
        ...
        rlDisableTexture();
#else
        // NOTE: If quads batch limit is reached, force a draw call and next
        // batch starts
        if (RLGL.State.vertexCounter >=
            RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
                    .elementCount *
        ...
        if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
            ...
            if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                ...
                if (!rlCheckRenderBatchLimit(
                    ...
                    RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                        .mode = RLGL.currentBatch
                                    ->draws[RLGL.currentBatch->drawCounter - 2]
                                    .mode;
                }
            }
...
#if (defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2) ||    \
     defined(GRAPHICS_API_OPENGL_SOFTWARE))
...
void rlActiveDrawBuffers(int count) {
    ...
    // dependent, it can be queried with glGet*() but it must be at least 8
    // GLint maxDrawBuffers = 0;
    // glGetIntegerv(GL_MAX_DRAW_BUFFERS, &maxDrawBuffers);

    if (count > 0) {
        if (count > 8)
            TRACELOG(RL_LOG_WARNING, "GL: Max color buffers limited to 8");
        else {
        ...
        TRACELOG(RL_LOG_WARNING, "GL: One color buffer active by default");
#endif
}
...
void rlglInit(int width, int height) {
    ...
    // render batch
    RLGL.State.currentShaderLocs[RL_SHADER_LOC_VERTEX_NORMAL] =
        RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL;
    ...
    if (result == 0) {
        TRACELOG(RL_LOG_ERROR,
                 "RLSW: Software renderer initialization failed!");
        exit(-1);
    }
#endif
...
void rlLoadExtensions(void *loader) {
    ...
    // Get supported extensions list
    GLint numExt = 0;
    const char **extList = (const char **)RL_CALLOC(
        512, sizeof(const char *)); // Allocate 512 strings pointers (2 KB)
    const char *extensions =
        (const char *)glGetString(GL_EXTENSIONS); // One big const string

    // NOTE: String duplication rquired because glGetString() returns a const
    // string
    int extensionsLength =
        (int)strlen(extensions); // Get extensions string size in bytes
    char *extensionsDup = (char *)RL_CALLOC(
        extensionsLength + 1,
        sizeof(char)); // Allocate space for copy with additional EOL byte
    ...
    for (int i = 0; i < numExt; i++) {
        ...
        if (strstr(extList[i], (const char *)"instanced_arrays") !=
            NULL) // Broad check for instanced_arrays
        {
            // Specific check
            ...
            } else if (strcmp(extList[i],
                              (const char *)"GL_EXT_instanced_arrays") ==
                       0) // EXT
            {
                glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
                ...
                glVertexAttribDivisor = (PFNGLVERTEXATTRIBDIVISOREXTPROC)((
                    rlglLoadProc)loader)("glVertexAttribDivisorEXT");
                              ...
                              (const char *)"GL_NV_instanced_arrays") ==
                       0) // NVIDIA GLES
            {
                glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
                    rlglLoadProc)loader)("glDrawArraysInstancedNV");
                glDrawElementsInstanced = (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)((
                    rlglLoadProc)loader)("glDrawElementsInstancedNV");
                glVertexAttribDivisor = (PFNGLVERTEXATTRIBDIVISOREXTPROC)((
                    rlglLoadProc)loader)("glVertexAttribDivisorNV");
            }

            // The feature will only be marked as supported if the elements from
            // GL_XXX_instanced_arrays are present
            ...
            if (strcmp(extList[i], (const char *)"GL_EXT_draw_instanced") ==
                ...
                glDrawElementsInstanced = (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)((
                    rlglLoadProc)loader)("glDrawElementsInstancedEXT");
                              ...
                              (const char *)"GL_NV_draw_instanced") == 0) {
                glDrawArraysInstanced = (PFNGLDRAWARRAYSINSTANCEDEXTPROC)((
                    rlglLoadProc)loader)("glDrawArraysInstancedNV");
                glDrawElementsInstanced = (PFNGLDRAWELEMENTSINSTANCEDEXTPROC)((
                    rlglLoadProc)loader)("glDrawElementsInstancedNV");
            ...
            // But the functions will at least be loaded if only
            // GL_XX_EXT_draw_instanced exist
            if ((glDrawArraysInstanced != NULL) &&
                (glDrawElementsInstanced != NULL) &&
                (glVertexAttribDivisor != NULL))
                RLGL.ExtSupported.instancing = true;
...
rlRenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements) {
    ...
    if (!isGpuReady) {
        TRACELOG(RL_LOG_WARNING, "GL: GPU is not ready to load data, trying to "
                                 "load before InitWindow()?");
        return batch;
    }
    ...
    //--------------------------------------------------------------------------------------------
    batch.vertexBuffer =
        (rlVertexBuffer *)RL_CALLOC(numBuffers, sizeof(rlVertexBuffer));

    for (int i = 0; i < numBuffers; i++) {
        batch.vertexBuffer[i].elementCount = bufferElements;

        batch.vertexBuffer[i].vertices = (float *)RL_CALLOC(
            bufferElements * 3 * 4,
            sizeof(float)); // 3 float by vertex, 4 vertex by quad
        batch.vertexBuffer[i].texcoords = (float *)RL_CALLOC(
            bufferElements * 2 * 4,
            sizeof(float)); // 2 float by texcoord, 4 texcoord by quad
        batch.vertexBuffer[i].normals = (float *)RL_CALLOC(
            bufferElements * 3 * 4,
            sizeof(float)); // 3 float by vertex, 4 vertex by quad
        batch.vertexBuffer[i].colors = (unsigned char *)RL_CALLOC(
            bufferElements * 4 * 4,
            sizeof(unsigned char)); // 4 float by color, 4 colors by quad
#if defined(GRAPHICS_API_OPENGL_33)
        batch.vertexBuffer[i].indices = (unsigned int *)RL_CALLOC(
            bufferElements * 6,
            sizeof(unsigned int)); // 6 int by quad (indices)
#endif
#if defined(GRAPHICS_API_OPENGL_ES2)
        batch.vertexBuffer[i].indices = (unsigned short *)RL_CALLOC(
            bufferElements * 6,
    ...
    for (int i = 0; i < numBuffers; i++) {
        ...
        glBufferData(GL_ARRAY_BUFFER, bufferElements * 3 * 4 * sizeof(float),
                     batch.vertexBuffer[i].normals, GL_DYNAMIC_DRAW);
        glEnableVertexAttribArray(
            RLGL.State.currentShaderLocs[RL_SHADER_LOC_VERTEX_NORMAL]);
        glVertexAttribPointer(
            RLGL.State.currentShaderLocs[RL_SHADER_LOC_VERTEX_NORMAL], 3,
            GL_FLOAT, 0, 0, 0);
    ...
    //--------------------------------------------------------------------------------------------
    batch.draws =
        (rlDrawCall *)RL_CALLOC(RL_DEFAULT_BATCH_DRAWCALLS, sizeof(rlDrawCall));
...
// Draw render batch
// NOTE: Batch is reseted and current buffer is updated (for multi-buffer
// config)
void rlDrawRenderBatch(rlRenderBatch *batch) {
    ...
    if (RLGL.State.vertexCounter > 0) {
        ...
        // TODO: If no data changed on the CPU arrays there is no need to
        // re-upload data to GPU, a flag can be used to detect changes but it
        // would imply keeping a copy buffer and memcmp() both, does it worth
        // it?
        ...
        // glBufferData(GL_ARRAY_BUFFER,
        // sizeof(float)*3*4*batch->vertexBuffer[batch->currentBuffer].elementCount,
        // batch->vertexBuffer[batch->currentBuffer].normals, GL_DYNAMIC_DRAW);
        // // Update all buffer
        ...
        // until GPU to finish its job To avoid waiting (idle), glBufferData()
        // can bee called first with NULL pointer before glMapBuffer() Doing
        // that, the previous data in PBO will be discarded and glMapBuffer()
    ...
    for (int eye = 0; eye < eyeCount; eye++) {
        ...
        if (RLGL.State.vertexCounter > 0) {
            ...
            if (RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_PROJECTION] !=
                -1) {
                glUniformMatrix4fv(
                    RLGL.State
                        .currentShaderLocs[RL_SHADER_LOC_MATRIX_PROJECTION],
                    1, false, rlMatrixToFloat(RLGL.State.projection));
            }

            // WARNING: For the following setup of the view, model, and normal
            // matrices, it is expected that transformations and rendering occur
            // between rlPushMatrix() and rlPopMatrix()

            if (RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_VIEW] != -1) {
                glUniformMatrix4fv(
                    RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_VIEW], 1,
                    false, rlMatrixToFloat(RLGL.State.modelview));
            }

            if (RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_MODEL] !=
                -1) {
                glUniformMatrix4fv(
                    RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_MODEL], 1,
                    false, rlMatrixToFloat(RLGL.State.transform));
            }

            if (RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_NORMAL] !=
                -1) {
                glUniformMatrix4fv(
                    RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_NORMAL],
                    1, false,
                    rlMatrixToFloat(rlMatrixTranspose(
                        rlMatrixInvert(RLGL.State.transform))));
            }
            ...
            if (RLGL.ExtSupported.vao)
                ...
                glBindBuffer(
                    ...
                    batch->vertexBuffer[batch->currentBuffer].vboId[4]);
            }
...
#if defined(GRAPHICS_API_OPENGL_33)
                    // The number of indices to be processed needs to be
                    // defined: elementCount*6 NOTE: The final parameter tells
                    // the GPU the offset in bytes from the start of the index
                    // buffer to the location of the first index to process
                    glDrawElements(
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format,
    ...
    if (mipmapCount > 1) {
        ...
        // Define the maximum number of mipmap levels to be used, 0 is base
        // texture size
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL, 0);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, mipmapCount - 1);

        // Check if the loaded texture with mipmaps is complete,
        // uncomplete textures will draw in black if mipmap filtering is
        // required
        // GLint complete = 0;
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer) {
    ...
    // WARNING: Specification only allows GL_DEPTH_COMPONENT32F for GL_FLOAT
    // type REF:
    // https://registry.khronos.org/OpenGL-Refpages/es3.0/html/glTexImage2D.xhtml
    if (RLGL.ExtSupported.maxDepthBits == 24)
    ...
    // WARNING: Ensure that the depth format is the one specified at rlsw
    // compilation
    glGenRenderbuffers(1, &id);
    glBindRenderbuffer(GL_RENDERBUFFER, id);
        ...
        for (int i = 0; i < 6 * mipmapCount; i++) {
            ...
            if (data == NULL) {
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) {
                    if ((format == RL_PIXELFORMAT_UNCOMPRESSED_R32) ||
                        ...
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,
                                     mipmapLevel, glInternalFormat, mipSize,
                                     mipSize, 0, glFormat, glType, NULL);
                } else
                    TRACELOG(RL_LOG_WARNING,
                ...
                if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
                    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,
                                 mipmapLevel, glInternalFormat, mipSize,
                                 mipSize, 0, glFormat, glType,
                                 (unsigned char *)dataPtr + face * dataSize);
                else
                    glCompressedTexImage2D(
                        GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, mipmapLevel,
                        glInternalFormat, mipSize, mipSize, 0, dataSize,
                        (unsigned char *)dataPtr + face * dataSize);
            }
            ...
            if (face == 5) {
                ...
                if (data != NULL)
                    dataPtr +=
                        dataSize * 6; // Increment data pointer to next mipmap
...
// WARNING: Not possible to know safely if internal texture format is the
// expected one...
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format) {
    ...
    // NOTE: Using texture id, some texture info can be retrieved (but not on
    ...
    // to get it from texture id Two possible Options: 1 - Bind texture to color
    // fbo attachment and glReadPixels() 2 - Create an fbo, activate it, render
    // quad with texture, glReadPixels() Using Option 1, care for texture format
    // on retrieval NOTE: This behaviour could be conditioned by graphic
    // driver...
    ...
    // Reading data as RGBA because FBO texture is configured as RGBA, despite
...
void rlCopyFramebuffer(int x, int y, int width, int height, int format,
                       void *pixels) {
#if defined(GRAPHICS_API_OPENGL_SOFTWARE)
    unsigned int glInternalFormat, glFormat, glType;
    rlGetGlTextureFormats(format, &glInternalFormat, &glFormat,
                          &glType); // Get OpenGL texture format
...
unsigned char *rlReadScreenPixels(int width, int height) {
    ...
    // NOTE: glReadPixels() returns image flipped vertically -> (0,0) is the
    // bottom left corner of the framebuffer WARNING: Getting alpha channel! Be
    // careful, it can be transparent if not cleared properly!
    glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, imgData);

    // Flip image vertically
    // NOTE: Alpha value has already been applied to RGB in framebuffer, not
    // needed anymore
    for (int y = height - 1; y >= height / 2; y--) {
        for (int x = 0; x < (width * 4); x += 4) {
            ...
            imgData[s + 3] = 255; // Set alpha component value to 255 (no
                                  // trasparent image retrieval)
...
// Attach color buffer texture to a framebuffer object (unloads previous
// attachment) NOTE: Attach type: 0-Color, 1-Depth renderbuffer, 2-Depth texture
void rlFramebufferAttach(unsigned int id, unsigned int texId, int attachType,
...
void rlUnloadFramebuffer(unsigned int id) {
    ...
    // parameter name REF: https://registry.khronos.org/webgl/specs/latest/1.0/
    int depthId = 0;
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
...
unsigned int rlLoadShader(const char *code, int type) {
    ...
    if (success == GL_FALSE) {
        ...
        case GL_COMPUTE_SHADER:
            TRACELOG(RL_LOG_WARNING,
                     "SHADER: Compute shaders not enabled. Define "
...
unsigned int rlLoadShaderProgram(const char *vsCode, const char *fsCode) {
    ...
    // recompile, assign the default shader program id
    if ((vertexShaderId == RLGL.State.defaultVShaderId) &&
        (fragmentShaderId == RLGL.State.defaultFShaderId))
        id = RLGL.State.defaultShaderId;
    else if ((vertexShaderId > 0) && (fragmentShaderId > 0)) {
        // One of or both shader are new, a new shader program needs to be
        // compiled
        ...
        &type, name);
                ...
                name[namelen] = 0;
                TRACELOG(RL_LOG_DEBUG, "SHADER: [ID %i] Active uniform (%s) set
        at location: %i", id, name, glGetUniformLocation(id, name));
            }
        }
        */
    }
...
unsigned int rlLoadShaderProgramEx(unsigned int vsId, unsigned int fsId) {
    ...
    // NOTE: There is no problem with binding a generic attribute index to an
    // attribute variable name that is never used; if some attrib name is no
    // found on the shader, it locations becomes -1
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_POSITION);
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD);
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_NORMAL);
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_COLOR,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_COLOR);
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_TANGENT,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_TANGENT);
    glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD2,
    ...
    glBindAttribLocation(programId,
                         RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEINDICES,
                         RL_DEFAULT_SHADER_ATTRIB_NAME_BONEINDICES);
    glBindAttribLocation(programId,
                         RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEWEIGHTS,
    ...
    if (success == GL_FALSE) {
        TRACELOG(RL_LOG_WARNING,
                 "SHADER: [ID %i] Failed to link shader program", programId);
        ...
        if (maxLength > 0) {
            ...
            TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Link error: %s",
                     programId, log);
            RL_FREE(log);
        }
        ...
        TRACELOG(RL_LOG_INFO,
                 "SHADER: [ID %i] Program shader loaded successfully",
                 programId);
    }
#endif
    return programId;
...
unsigned int rlLoadShaderProgramCompute(unsigned int csId) {
    ...
    TRACELOG(
        RL_LOG_WARNING,
        "SHADER: Compute shaders not supported, enable GRAPHICS_API_OPENGL_43");
#endif
...
void rlUnloadShader(unsigned int id) {
    ...
    TRACELOG(RL_LOG_INFO,
             "SHADER: [ID %i] Unloaded shader data from VRAM (GPU)", id);
#endif
}
...
#else
    TRACELOG(RL_LOG_WARNING,
             "SSBO: SSBO not enabled. Define GRAPHICS_API_OPENGL_43");
#endif
...
void rlBindImageTexture(unsigned int id, unsigned int index, int format,
    ...
    glBindImageTexture(index, id, 0, 0, 0,
                       ...
                       glInternalFormat);
#else
    TRACELOG(RL_LOG_WARNING, "TEXTURE: Image texture binding not enabled. "
                             "Define GRAPHICS_API_OPENGL_43");
#endif
}
...
void rlLoadDrawQuad(void) {
    ...
    glVertexAttribPointer(RL_DEFAULT_SHADER_ATTRIB_LOCATION_POSITION, 3,
                          GL_FLOAT, GL_FALSE, 5 * sizeof(float),
                          (void *)0); // Positions
    glEnableVertexAttribArray(RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD);
    glVertexAttribPointer(RL_DEFAULT_SHADER_ATTRIB_LOCATION_TEXCOORD, 2,
                          GL_FLOAT, GL_FALSE, 5 * sizeof(float),
...
void rlLoadDrawCube(void) {
    ...
    glVertexAttribPointer(RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL, 3, GL_FLOAT,
...
static void rlLoadShaderDefault(void) {
...
#if defined(GRAPHICS_API_OPENGL_ES3)
    "#version 300 es                    \n"
    "precision mediump float;           \n" // Precision required for OpenGL ES3
                                            // (WebGL 2)
    "in vec2 fragTexCoord;              \n"
    "in vec4 fragColor;                 \n"
    "out vec4 finalColor;               \n"
    ...
    if (RLGL.State.defaultShaderId > 0) {
        ...
        RLGL.State.defaultShaderLocs[RL_SHADER_LOC_VERTEX_POSITION] =
            glGetAttribLocation(RLGL.State.defaultShaderId,
                                RL_DEFAULT_SHADER_ATTRIB_NAME_POSITION);
        ...
        RLGL.State.defaultShaderLocs[RL_SHADER_LOC_MATRIX_MVP] =
            glGetUniformLocation(RLGL.State.defaultShaderId,
...
static int rlGetPixelDataSize(int width, int height, int format) {
    ...
    if ((format >= RL_PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) &&
        ...
        dataSize =
            (int)(bytesPerPixel * width * height); // Total data size in bytes
    }

...
static int lock_move_count, y_max, score, combo;
static int level, lines;
static bool hard_drop, b2b;
...
bool input(void) {
    ...
    static double paused_timer = 0;

    if (IsKeyPressed(KEY_P)) {
        paused = !paused;
        if (paused) {
            paused_timer = GetTime();
        } else if (paused_timer) {
            double elapsed_time = GetTime() - paused_timer;
            last_move_time[DOWN] += elapsed_time;
            if (curr_shape_state)
                curr_shape_state_timer += elapsed_time;
            paused_timer = 0;
        }
    }
    ...
    if (IsKeyDown(KEY_DOWN)) { // Soft drop
        if (move_shape(&curr_shape, DOWN, SOFT_DROP_DELAY))
            score += 1;
    }
...
void update_tetris(bool new_move) {
    struct Shape down_check = curr_shape;
    down_check.pos.y++;
    bool is_grounded = shape_collides(&down_check).hit;

    printf("\rLevel: %d, Lines %d, Combos %d, Score %d    ", level, lines,
           combo, score);
    fflush(stdout);
    ...
    if (is_grounded || hard_drop) {
        ...
        // Guarantees either hard drop || STATE_LOCKING
            ...
            int lines_cleared = clear_and_compress();
